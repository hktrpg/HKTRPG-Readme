...
### 與 `.forward`、`.re` 搭配（Discord）

跑團時常見做法：把按鈕放在準備頻道，結果轉發到公開頻道。

1. 產生按鈕：`.ch button` 或 `.char button 角色名`（或 GM 用 [`.re`](../yao-qiu-zhi-tou.md) 要求玩家點擊擲骰）
2. 複製該則訊息連結
3. 到目標頻道輸入 [`.forward 訊息連結`](../../xi-tong-gong-ju/discord-an-niu-zhuan-fa.md)

之後點擊按鈕，結果會出現在已設定的轉發頻道。轉發上限見 [開發支援（VIP）](../../kai-shi-shi-yong/kai-fa-zhi-yuan.md#vip-權益對照)。

若要對外分享「好看的角色介紹圖」而非 Bot 狀態卡，可用網頁 [TRPG 名片與喜好卡](../trpg-ming-pian-yu-xi-hao-ka-gong-neng.md) 匯出 PNG。
